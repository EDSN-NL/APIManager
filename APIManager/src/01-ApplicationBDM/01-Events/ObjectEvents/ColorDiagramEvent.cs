...
namespace Plugin.ApplicationBDM.Events
{
    class ColorDiagramEvent : ObjectEventImplementation
    {
        ...
        internal override bool HandleEvent(ObjectEventType eventType, ObjectType objectType, object eventObject, Diagram targetDiagram)
        {
            ...
            int newBGColor = -1;
            int newFontColor = -1;
            string protectColorTagName = ContextSlt.GetContextSlt().GetConfigProperty(_ColorProtectionTag);
            ...
            if (IsValidContext() && openedDiagram != null)
            {
                ...
                foreach (DiagramClassRepresentation rep in openedDiagram.GetRepresentations())
                {
                    ...
                    string protectColorTag = rep.AssociatedClass.GetTag(protectColorTagName);
                    bool protectColor = !string.IsNullOrEmpty(protectColorTag) && string.Compare(protectColorTag, "true", true) == 0;
                    if (!protectColor && newBGColor != -1 && newFontColor != -1 && (currentBGColor != newBGColor || currentFontColor != newFontColor))
                    {
